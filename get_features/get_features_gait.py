...
def get_features_gait():
    ...
    for i in range(7):
            ...
            i = go_idx
            while COM_values_x[i] < neutral_x + 0.2: i+=1
            start_time_idx = i
            while COM_values_x[i] < COM_values_x[start_time_idx] + time_dist: i+=1
            end_time_idx = i
              
            ## GET FEATURES ###############################################################
            mean_speed = np.mean(COM_speeds_x[start_idx:end_idx])
            time_3m = time[end_time_idx] - time[start_time_idx]
            
            gait_features = {"mean_speed": mean_speed, "time_3m": time_3m}
            filepath = 'features/gait_features_' + record_id + '.json'
            with open(filepath, 'w') as json_file:
              json.dump(gait_features, json_file)
    print('Finished')