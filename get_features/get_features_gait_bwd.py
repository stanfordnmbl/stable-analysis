...
def get_features_gait_bwd():
    print("Running gait-bwd... ")
    record_ids = ['42', '68', '140', '144', '167', '179', '205']
    for i in range(7):
            record_id = record_ids[i]
            gait_bwd_features = {}  
            trial_names = ["gait-bwd"]
            ...
            wrist_peak_idx,_ = find_peaks(signal, prominence=0.5) # adjusted from 0.5 since 6 subjects failed
            ...
            gait_bwd_features = {"mean_speed": mean_speed}
            filepath = 'features/gait_bwd_features_' + record_id + '.json'
            with open(filepath, 'w') as json_file:
              json.dump(gait_bwd_features, json_file)
    print('Finished')