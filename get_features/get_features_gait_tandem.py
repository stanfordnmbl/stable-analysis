...
def get_features_gait_tandem():
    print('Running gait_tandem ... ')
    record_ids = ['42', '68', '140', '144', '167']
    for i in range(5):
            record_id = record_ids[i]
            gait_tandem_features = {}  
            trial_names = ["gait-tandem"]
            ...
            c7_std = np.std(marker_dict['markers']['C7_study'][:,2][start_idx:end_idx])
            
            gait_tandem_features = {"mean_speed": mean_speed, "c7_std": c7_std}
            
            filepath = 'features/gait_tandem_features_' + record_id + '.json'
            with open(filepath, 'w') as json_file:
              json.dump(gait_tandem_features, json_file)
    print('Finished')